...
public class MessageEntity {
    ...
    @JoinColumn(name = "mno", insertable = false, updatable = false, nullable = false)  // 송신자 외래 키
    @OnDelete(action = OnDeleteAction.NO_ACTION) // 송신&수신자 삭제시 같이 삭제
    private MemberEntity sendermno; // 송신자

    @ManyToOne(cascade = CascadeType.ALL)
    @JoinColumn(name = "mno", insertable = false, updatable = false, nullable = false)  // 수신자 외래 키
    @OnDelete(action = OnDeleteAction.NO_ACTION) // 송신&수신자 삭제시 같이 삭제
